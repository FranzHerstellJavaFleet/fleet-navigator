  ...
  <div class="min-h-screen p-6 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
    ...
    <div v-if="loading" class="flex items-center justify-center h-full">
      <div class="text-center">
        ...
        <p class="text-gray-400 mt-4">Lade Officer-Details...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="flex items-center justify-center h-full">
      <div class="text-center">
        ...
        <button
          @click="$router.push({ name: 'fleet-officers' })"
    ...
    <div v-else-if="officer" class="w-full max-w-7xl mx-auto">
      <div class="
        bg-gray-900
      ...
      ">
        ...
        ">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h2 class="text-2xl font-bold text-white">{{ officer.name }}</h2>
              <div class="flex items-center gap-3 mt-1">
                <p class="text-sm text-gray-400">{{ officer.description }}</p>
                <span
        ...
        <div class="p-6 min-h-[600px]">
          ...
          <div v-if="activeTab === 'hardware'">
            <div v-if="stats && officer.status === 'ONLINE'" class="space-y-4">
              <!-- System Info -->
            ...
            <div v-else-if="officer.status === 'OFFLINE'" class="
            ...
            ">
              ...
              <h3 class="text-xl font-semibold text-red-400 mb-2">Officer ist offline</h3>
              <p class="text-sm text-gray-400">
                Keine Hardware-Daten verfügbar. Starte den Officer, um Daten zu sehen.
              </p>
            </div>
            <div v-else class="
            ...
            <div class="bg-gray-800/50 p-4 rounded-xl border border-gray-700/50">
              ...
              <div class="space-y-3">
                ...
                <button
                  ...
                  :disabled="analyzing || officer.status !== 'ONLINE'"
        ...
        ">
          <div class="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span v-if="officer.lastHeartbeat">
              Letztes Heartbeat: {{ formatTime(officer.lastHeartbeat) }}
            </span>
          </div>
          <div class="flex gap-3">
            <button
              @click="pingOfficer"
              :disabled="officer.status !== 'ONLINE' || pinging"
              class="
...
import OfficerTerminal from '../../components/OfficerTerminal.vue'
...
const loadOfficerData = async () => {
  try {
    ...
    const officerResponse = await axios.get(`/api/fleet-officer/officers/${officerId}`)
    officer.value = officerResponse.data

    // Load stats
    try {
      const statsResponse = await axios.get(`/api/fleet-officer/officers/${officerId}/stats`)
    ...
    error.value = 'Officer konnte nicht geladen werden'
    loading.value = false
  }
}

// Load available Ollama models when view opens
async function loadAvailableModels() {
  loadingModels.value = true
  try {
    const response = await axios.get('/api/fleet-officer/ollama-models')
    availableModels.value = response.data
    console.log('✅ Loaded Ollama models:', response.data)
...
async function pingOfficer() {
  pinging.value = true
  try {
    await axios.post(`/api/fleet-officer/officers/${officer.value.officerId}/ping`)
    success('Ping erfolgreich gesendet')
  } catch (err) {
    console.error('Failed to ping officer:', err)
...
async function startLogAnalysis() {
  ...
  try {
    ...
    const response = await axios.post(
      `/api/fleet-officer/officers/${officer.value.officerId}/analyze-log`,
      {
    ...
    eventSource.addEventListener('error', (event) => {
      ...
      eventSource.close()
      analyzing.value = false
      errorToast('Fehler bei der Log-Analyse')
    })
    ...
    if (officer.value) {
      try {
        ...
        const [officerResponse, statsResponse] = await Promise.all([
          axios.get(`/api/fleet-officer/officers/${officerId}`),
          axios.get(`/api/fleet-officer/officers/${officerId}/stats`).catch(() => ({ data: null }))
        ])
        officer.value = officerResponse.data
        stats.value = statsResponse.data
      } catch (err) {
        console.error('Failed to refresh officer data:', err)
      }
    }
  }, 5000)
})
