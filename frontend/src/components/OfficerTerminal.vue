    ...
    <div class="bg-black/70 rounded-xl border border-gray-700/50 font-mono text-xs min-h-[400px] max-h-[600px] overflow-y-auto custom-scrollbar">
      ...
      <div class="flex items-center justify-between px-4 py-2 bg-gray-800/50 border-b border-gray-700 sticky top-0 z-10">
        <div class="flex items-center gap-2 text-gray-400">
          ...
          <span class="font-semibold">fleet-officer@{{ officerId }}</span>
        </div>
        <div class="flex gap-2">
          <button
            @click="clearTerminal"
...
async function loadCommandHistory() {
  try {
    const response = await axios.get(`/api/fleet-officer/officers/${props.officerId}/command-history`)
...
async function executeCommand(command, args) {
  ...
  try {
    ...
    const response = await axios.post(
      `/api/fleet-officer/officers/${props.officerId}/execute`,
      {
        command: command,
        args: args,
